...
struct ShuntingYard{
    ...
    using curr = pair<S, D>;
    using c_t = std::conditional_t<is_alphanumeric(c),
                Alnum_t,
                std::conditional_t<is_operation(c),
                Operation_t,
                std::conditional_t<(is_left_paren(c)),
                LeftParen_t,
                RightParen_t>>>;

    using next = std::conditional_t<Pos < Str.size(),
        typename ShuntingYardStep<c, curr, c_t>::next,
        curr>;

    using S_next = first_t<next>;
    using D_next = second_t<next>;

    using type = typename std::conditional_t<Pos < Str.size(),
          ShuntingYard<Pos+1, Str, S_next, D_next>,
          curr>::type;
};
