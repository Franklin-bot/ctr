...
struct ShuntingYardImpl {
  ...
  using type = typename std::conditional_t<
    (Pos < Str.size()), ShuntingYardImpl<Pos + 1, Str, S_next, D_next>, curr>::type;
};

// base case
template <size_t Pos, fixed_string Str, class S, class D>
requires(Pos >= Str.size())
struct ShuntingYardImpl<Pos, Str, S, D> {
  using P_final = PopAll_t<S, D>;
  using type = second_t<P_final>;
};

template <fixed_string Str>
using ShuntingYard = typename ShuntingYardImpl<0, Str>::type;
